...
class TransactionController extends AbstractController
{
    ...
    public function new(Request $request, TransactionRepository $transactionRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash("success","La transaction a bien été ajoutée avec succès.");
    ...
    public function edit(Request $request, Transaction $transaction, TransactionRepository $transactionRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $transactionRepository->save($transaction, true);
            $this->addFlash("success","La transaction a bien été modifiée avec succès.");
            return $this->redirectToRoute('app_transaction_edit', ['id' => $transaction->getId()], Response::HTTP_SEE_OTHER);
        }
    ...
    #[Route('/{id}/action/delete', name: 'app_transaction_form_delete', methods: ['GET', 'POST'])]
    public function formDelete(Transaction $transaction): Response
    {
        $form = $this->createForm(TransactionType::class, $transaction);

        return $this->renderForm('transaction/delete.html.twig', [
            'transaction' => $transaction,
            'form' => $form,
        ]);
    }
    ...
    public function delete(Request $request, Transaction $transaction, TransactionRepository $transactionRepository): Response
    {
        ...
        $this->addFlash("success","La transaction a bien été supprimée avec succès.");
